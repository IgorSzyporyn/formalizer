...
function getMaxDepth({
  ...
  if (previousItem && activeItem.type) {
    const accepts = previousItem.accepts || [];
    const acceptedItem = accepts.includes(activeItem.type);
    const delta = acceptedItem ? 1 : 0;

    return previousItem.depth + delta;
  }
...
export function buildTree(flattenedItems: FlattenedItem[]): TreeItems {
  ...
  for (const item of items) {
    ...
    nodes[id] = { id, items, accepts, modelId: item.modelId };
